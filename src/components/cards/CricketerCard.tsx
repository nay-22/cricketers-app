...
const CricketerCard: FC<TPlayer> = ({ id, name, description, type, points, rank, dob }) => {
    return (
        <div className="rounded-xl shadow-2xl w-full">
            <div className="bg-gray-300 rounded-t-xl p-2 flex items-center justify-between">
                ...
                {dob && <div className="font-semibold text-gray-600">{new Date(dob).toLocaleDateString()}</div>}
            </div>
            <div className="bg-slate-200">
                ...
                <div className="p-2 bg-gray-300 rounded-b-2xl flex items-center justify-start gap-2">
                    ...
                    {points && <Chip className="bg-slate-500 text-white">Points: {points}</Chip>}
